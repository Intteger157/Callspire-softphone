...
namespace Softphone.AppHost.ViewModels
{
    ...
    public sealed class SettingsViewModel : ObservableObject
    {
        ...
        /// <summary>Where post-call Kommo upload runs (WPF "Kommo source" toggle → <see cref="AppSettings.AmoCrmRecordingUploadSource"/>).</summary>
        public static IReadOnlyList<ChoiceOption> KommoSourceOptions { get; } = new[]
        {
            new ChoiceOption("local", "This computer (Kommo API)"),
            new ChoiceOption("gateway", "PBX Gateway"),
        };
        ...
        // SIP codec (WPF Audio panel parity: AudioCodecComboBox / SampleRateComboBox / AppSettings.AudioBitrate)
        public static IReadOnlyList<ChoiceOption> SipCodecOptions { get; } = new[]
        {
            new ChoiceOption("PCMU", "G.711 μ-law (PCMU) - Standard"),
            new ChoiceOption("PCMA", "G.711 A-law (PCMA) - Standard"),
            new ChoiceOption("G722", "G.722 - High Quality"),
            new ChoiceOption("opus", "Opus - Best Quality"),
        };
        public static IReadOnlyList<int> SipSampleRateOptions { get; } = new[] { 8000, 16000 };

        private ChoiceOption _sipCodec = SipCodecOptions[0];
        private int _sipSampleRate = 16000;
        private int _sipOpusBitrate = 64000;

        public ChoiceOption SipCodec { get => _sipCodec; set { if (Set(ref _sipCodec, value)) OnPropertyChanged(nameof(SipCodecIsOpus)); } }
        public bool SipCodecIsOpus => string.Equals(_sipCodec.Key, "opus", StringComparison.OrdinalIgnoreCase);
        public int SipSampleRate { get => _sipSampleRate; set => Set(ref _sipSampleRate, value); }
        public int SipOpusBitrate { get => _sipOpusBitrate; set => Set(ref _sipOpusBitrate, value); }
        ...
        private ChoiceOption _kommoSource = KommoSourceOptions[0];
        public ChoiceOption KommoSource { get => _kommoSource; set { if (Set(ref _kommoSource, value)) OnPropertyChanged(nameof(KommoIsGateway)); } }
        public bool KommoIsGateway => _kommoSource.Key == "gateway";
        ...
        private void Load(AppSettings s)
        {
            ...
            SipCodec = Pick(SipCodecOptions, s.AudioCodec, "PCMU");
            SipSampleRate = s.AudioSampleRate > 0 ? s.AudioSampleRate : 16000;
            SipOpusBitrate = s.AudioBitrate > 0 ? s.AudioBitrate : 64000;
            ...
            KommoSource = Pick(KommoSourceOptions, s.AmoCrmRecordingUploadSource, "local");
        ...
        private void ApplyTo(AppSettings s)
        {
            ...
            s.AudioCodec = SipCodec.Key;
            s.AudioSampleRate = SipSampleRateOptions.Contains(SipSampleRate) ? SipSampleRate : 16000;
            s.AudioBitrate = SipOpusBitrate > 0 ? SipOpusBitrate : 64000;
            if (!OperatingSystem.IsWindows())
            {
            ...
            s.AmoCrmRecordingUploadSource = KommoSource.Key;
            s.AmoCrmConnectionSource = KommoSource.Key;
