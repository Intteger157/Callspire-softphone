        ...
        public void PlayRingbackTone()
        {
            lock (_lockObject)
            {
                ...
                try
                {
                    ...
                    _waveOut.Play();
                    _isPlaying = true;
                    MainWindow.Log("[ToneGenerator] Ringback tone started");
                }
                catch (Exception ex)
                {
                    _isPlaying = false;
                    MainWindow.Log($"[ToneGenerator] Error playing ringback tone: {ex.Message}");
        ...
        public void PlayBusyTone()
        {
            lock (_lockObject)
            {
                ...
                try
                {
                    ...
                    _waveOut.Play();
                    _isPlaying = true;
                    MainWindow.Log("[ToneGenerator] Busy tone started");
                }
                catch (Exception ex)
                {
                    _isPlaying = false;
                    MainWindow.Log($"[ToneGenerator] Error playing busy tone: {ex.Message}");
            ...
            lock (_lockObject)
            {
                try
                {
                    if (_waveOut != null)
                    {
                        ...
                        _waveOut = null;
                        _isPlaying = false;
                        MainWindow.Log("[ToneGenerator] Tone stopped");
                    }
                ...
                {
                    _isPlaying = false;
                    MainWindow.Log($"[ToneGenerator] Error stopping tone: {ex.Message}");
