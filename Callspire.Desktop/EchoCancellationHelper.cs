...
namespace Softphone
{
    ...
    ///
    /// Платформенная логика AEC: на Windows используется native webrtc_apm.dll, на остальных
    /// платформах — SoftwareAec (выбор через Softphone.Audio.EchoCancellerFactory). Этот класс
    /// настраивает только WASAPI Communications-режим и на не-Windows платформах не делает ничего.
    /// </summary>
    public static class EchoCancellationHelper
    {
        ...
        public static void EnableEchoCancellation()
        {
            ...
            {
                // macOS/Linux: AEC обеспечивается SoftwareAec в аудио-пайплайне PortAudio
                // (см. DesktopAudioDeviceFactory) — WASAPI Communications-режим недоступен.
                MainWindow.Log("[EchoCancellationHelper] Non-Windows platform — WASAPI Communications mode skipped (SoftwareAec is wired in the PortAudio pipeline)");
                return;
            }
        ...
        {
            // На не-Windows платформах эхоподавление всегда доступно через SoftwareAec.
