...
namespace Softphone
{
    ...
    public class PortAudioAudioSource : IAudioSource, IDisposable
    {
        ...
        /// <summary>Tap of raw 8 kHz mono PCM before encoding (used for call recording).</summary>
                ...
                while (!cancellationToken.IsCancellationRequested && _isStarted && !_isPaused)
                {
                    ...
                    try
                    {
                        ...
                        if (IsSuccess(readResult))
                        {
                            ...
                            // AEC: убираем эхо из near-end (микрофон) кадра, in-place
                            try { EchoCanceller?.ProcessCapture(monoBuffer, monoBuffer.Length); } catch { }

                            // Raw PCM tap для записи звонка (после AEC, до кодека)
                            try { OnRawPcmFrameTap?.Invoke(monoBuffer); } catch { }
