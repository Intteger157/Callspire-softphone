...
namespace Softphone
{
    ...
    /// </summary>
    public class RtpCallRecorder : CallRecorderBase, Softphone.Audio.IRtpCallRecorder
    {
        ...
        public override void StartRecording(string phoneNumber, DateTime callStartTime, string recordingsDirectory)
        {
            ...
            lock (_lockObject)
            {
                ...
                try
                {
                    ...
                    {
                        AppLog.Log($"[RtpCallRecorder] ERROR: _pcmInPath is null or empty!");
                        _isRecording = false;
                        return;
                    }
                    ...
                    {
                        AppLog.Log($"[RtpCallRecorder] ERROR: _pcmOutPath is null or empty!");
                        _isRecording = false;
                        return;
                    }
                    ...
                    AppLog.Log($"[RtpCallRecorder] PCM streams created: Inbound={_pcmInPath}, Outbound={_pcmOutPath} (s16le 48k mono)");
                    ...
                    AppLog.Log($"[RtpCallRecorder] MuLaw selftest (NAudio.Codecs): 0xFF->{t1}, 0x7F->{t2}, 0x00->{t3}");
                }
                catch (Exception ex)
                {
                    AppLog.Log($"[RtpCallRecorder] Error starting recording: {ex.Message}");
                    _isRecording = false;
                    _pcmInPath = null;
        ...
        protected override void GenerateRecordingPaths(string baseFileName, string recordingsDirectory)
        {
            ...
            AppLog.Log($"[RtpCallRecorder] Recording paths: PCM_In={_pcmInPath}, PCM_Out={_pcmOutPath}, WAV={_recordingFilePath}");
        }
        ...
        public void ProcessInboundRtp(IPEndPoint remoteEndPoint, uint ssrc, uint seqnum, uint timestamp, int payloadID, bool marker, byte[] payload)
        {
            ...
            {
                AppLog.Log($"[RtpCallRecorder] First inbound RTP packet from {remoteEndPoint}");
            }
            else if (_inboundRtpPacketsSeen <= 10 && _inboundRtpPacketsSeen % 5 == 0)
            {
                AppLog.Log($"[RtpCallRecorder] Inbound RTP packet #{_inboundRtpPacketsSeen} from {remoteEndPoint}");
            }
            ...
            {
                AppLog.Log($"[RtpCallRecorder] Error processing inbound RTP: {ex.Message}");
            }
        }
        ...
        public void ProcessOutboundRawPcm48k(short[] pcm48k)
        {
                ...
                lock (_lockObject)
                {
                    if (_pcmOutStream != null && _isRecording)
                    {
                        ...
                        if (_outboundRtpPacketsSeen == 1)
                        {
                            ...
                            AppLog.Log($"[RtpCallRecorder][OUT] First 48k PCM chunk: samples={pcm48k.Length}, " +
                                $"peak={peak}, rms={rmsVal}, bytes={bytes.Length}");
                        }
                        else if (_outboundRtpPacketsSeen % 50 == 0)
                        {
                            AppLog.Log($"[RtpCallRecorder][OUT] 48k PCM chunks={_outboundRtpPacketsSeen}, " +
            ...
            {
                AppLog.Log($"[RtpCallRecorder][OUT] Error writing 48k PCM chunk #{_outboundRtpPacketsSeen}: {ex.Message}");
            }
        }
        ...
        public void ProcessOutboundRtpPayload(byte payloadType, byte[] payload, int sourceRateHz)
        {
            if (!_isRecording || _pcmOutStream == null)
            {
                ...
                {
                    AppLog.Log($"[RtpCallRecorder] ProcessOutboundRtpPayload called but not recording: isRecording={_isRecording}, pcmOutStream={_pcmOutStream == null}");
                }
                return;
            }
            ...
            if (payload == null || payload.Length == 0)
            {
                if (_outboundRtpPacketsSeen < 3)
                {
                    AppLog.Log($"[RtpCallRecorder] ProcessOutboundRtpPayload called with empty payload");
                }
                return;
            }
            ...
            if (_outboundRtpPacketsSeen == 1)
            {
                AppLog.Log($"[RtpCallRecorder][OUT] First outbound RTP packet: payloadType={payloadType}, payloadSize={payload.Length}, sourceRate={sourceRateHz}Hz");
            }
            else if (_outboundRtpPacketsSeen <= 10)
            {
                AppLog.Log($"[RtpCallRecorder][OUT] Outbound RTP packet #{_outboundRtpPacketsSeen}: payloadType={payloadType}, payloadSize={payload.Length}");
            }
            else if (_outboundRtpPacketsSeen % 50 == 0)
            {
                AppLog.Log($"[RtpCallRecorder][OUT] Outbound RTP packet #{_outboundRtpPacketsSeen}: payloadType={payloadType}, payloadSize={payload.Length}");
            ...
            try
            {
                ...
                if (decoded.Pcm48k == null || decoded.Pcm48k.Length == 0)
                {
                    ...
                    {
                        AppLog.Log($"[RtpCallRecorder][OUT] Decoded PCM is empty for packet #{_outboundRtpPacketsSeen}");
                    }
                    return;
                }
                ...
                lock (_lockObject)
                {
                    if (_pcmOutStream != null && _isRecording)
                    {
                        ...
                        if (_outboundRtpPacketsSeen == 1)
                        {
                            AppLog.Log($"[RtpCallRecorder][OUT] Written first outbound PCM: {decoded.Pcm48k.Length} bytes, position: {positionBefore} -> {_pcmOutStream.Position}");
                        }
                    }
                }
            }
            ...
            {
                AppLog.Log($"[RtpCallRecorder][OUT] Error processing outbound RTP packet #{_outboundRtpPacketsSeen}: {ex.Message}, stack: {ex.StackTrace}");
            }
        }
        ...
        public void ProcessOutboundSamples(AudioSamplingRatesEnum rate, uint durationMs, short[] samples)
        {
            if (samples == null || samples.Length == 0)
            {
                ...
                {
                    AppLog.Log($"[RtpCallRecorder][OUT] ProcessOutboundSamples: WARNING - samples is null or empty");
                }
                return;
            }
            ...
            lock (_lockObject)
            {
                if (!_isRecording)
                {
                    if (_outboundPcmChunksSeen == 0)
                    {
                        AppLog.Log($"[RtpCallRecorder][OUT] ProcessOutboundSamples: WARNING - not recording (isRecording={_isRecording})");
                    }
                    return;
                }
                ...
                if (_pcmOutStream == null)
                {
                    if (_outboundPcmChunksSeen == 0)
                    {
                        AppLog.Log($"[RtpCallRecorder][OUT] ProcessOutboundSamples: ERROR - _pcmOutStream is null! Path={_pcmOutPath}");
                    }
                    return;
                }
                ...
                try
                {
                    ...
                    if (isFirstCall || _outboundPcmChunksSeen <= 10)
                    {
                        ...
                        AppLog.Log($"[RtpCallRecorder][OUT] chunk #{_outboundPcmChunksSeen}: rate={sampleRate}->48k, " +
                    ...
                    {
                        AppLog.Log($"[RtpCallRecorder][OUT] chunks={_outboundPcmChunksSeen}, bytes={_pcmOutStream.Position}");
                    }
                }
                catch (Exception ex)
                {
                    AppLog.Log($"[RtpCallRecorder] ERROR writing outbound PCM: {ex.Message}, stack: {ex.StackTrace}");
                }
            }
        }
        ...
        public void ProcessOutboundPcm(byte[] pcmData, int sampleRate)
        {
            ...
            if (pcmData == null || pcmData.Length == 0)
            {
                ...
                {
                    AppLog.Log($"[RtpCallRecorder][MIC] ProcessOutboundPcm: WARNING - pcmData is null or empty (chunks seen={_outboundPcmChunksSeen})");
                }
                return;
            }
            ...
            lock (_lockObject)
            {
                if (!_isRecording)
                {
                    if (isFirstCall || _outboundPcmChunksSeen <= 3)
                    {
                        AppLog.Log($"[RtpCallRecorder][MIC] ProcessOutboundPcm: WARNING - not recording (isRecording={_isRecording}, chunks seen={_outboundPcmChunksSeen})");
                    }
                    return;
                }
                ...
                if (_pcmOutStream == null)
                {
                    if (isFirstCall || _outboundPcmChunksSeen <= 3)
                    {
                        AppLog.Log($"[RtpCallRecorder][MIC] ProcessOutboundPcm: ERROR - _pcmOutStream is null! Path={_pcmOutPath}, chunks seen={_outboundPcmChunksSeen}");
                    }
                    return;
                }
                ...
                try
                {
                    ...
                    if (sampleRate != SampleRate)
                    {
                        if (isFirstCall)
                        {
                            AppLog.Log($"[RtpCallRecorder][MIC] Resampling from {sampleRate}Hz to {SampleRate}Hz");
                        }
                    ...
                    if (isFirstCall || _outboundPcmChunksSeen <= 10)
                    {
                        ...
                        if (samplesCount > 0)
                        {
                            ...
                            AppLog.Log($"[RtpCallRecorder][MIC] ProcessOutboundPcm #{_outboundPcmChunksSeen}: " +
                    ...
                    {
                        AppLog.Log($"[RtpCallRecorder] ProcessOutboundPcm #{_outboundPcmChunksSeen}: still recording, streamPosition={_pcmOutStream.Position} bytes");
                    }
                }
                catch (Exception ex)
                {
        ...
        private DecodedPcm DecodeRtpPayloadEx(byte[] payload, int payloadID, int targetSampleRate)
        {
            try
            {
                ...
                switch (payloadID)
                {
                    ...
                    case 9: // G.722 - 16kHz аудио (RTP timestamps могут быть 8kHz)
                        ...
                        if (shouldLog)
                        {
                            AppLog.Log($"[RtpCallRecorder] G.722 (payloadID=9) not implemented, SKIPPING audio (payloadSize={payload.Length})");
                        }
                        return new DecodedPcm(Array.Empty<byte>(), 0);
                    ...
                    case 13: // Comfort Noise (CN)
                        if (shouldLog)
                        {
                            AppLog.Log($"[RtpCallRecorder] Comfort noise payload {payloadID} skipped (payloadSize={payload.Length})");
                        }
                        return new DecodedPcm(Array.Empty<byte>(), 0);
                    ...
                    case 101: // DTMF telephone-event (RFC 4733)
                        if (shouldLog)
                        {
                            AppLog.Log($"[RtpCallRecorder] DTMF/telephone-event payload {payloadID} skipped (payloadSize={payload.Length})");
                        }
                        return new DecodedPcm(Array.Empty<byte>(), 0);
                    ...
                    default:
                        ...
                        if (shouldLog)
                        {
                            AppLog.Log($"[RtpCallRecorder] Unknown payload ID {payloadID}, SKIPPING audio (payloadSize={payload.Length})");
                        }
                        return new DecodedPcm(Array.Empty<byte>(), 0);
                }
                ...
                if (shouldLog)
                {
                    AppLog.Log($"[RtpCallRecorder] Decoder selected: payloadID={payloadID} -> {codecName}, " +
            ...
            {
                AppLog.Log($"[RtpCallRecorder] Error decoding RTP payload (payloadID={payloadID}): {ex.Message}");
                return new DecodedPcm(Array.Empty<byte>(), 0);
            }
        }
        ...
        private byte[] DecodeG711MuLaw(byte[] muLawData, int targetSampleRate, bool shouldLog = false)
        {
            ...
            if (shouldLog)
            {
                ...
                AppLog.Log($"[RtpCallRecorder] MuLaw decode test: raw first10 samples peak={pcmSamples.Take(10).Max(s => Math.Abs(s))}, " +
        ...
        private byte[] DecodeOpus(byte[] opusData, int targetSampleRate)
        {
            try
            {
                ...
                lock (_opusDecoderLock)
                {
                    ...
                    if (decoder == null)
                    {
                        ...
                        AppLog.Log($"[RtpCallRecorder] Created Opus decoder for inbound stream, sample rate: {targetSampleRate}");
                    }
                    ...
                    if (decodedSamples <= 0)
                    {
                        AppLog.Log($"[RtpCallRecorder] Opus decode returned {decodedSamples} samples");
                        return new byte[0];
                    }
            ...
            {
                AppLog.Log($"[RtpCallRecorder] Error decoding Opus: {ex.Message}");
                return new byte[0];
            }
        }
        ...
        private short[] ResampleShortArray(short[] source, int sourceRate, int targetRate, bool shouldLog = false)
        {
            ...
            if (shouldLog)
            {
                AppLog.Log($"[RtpCallRecorder] Resampling: {sourceSamples} samples @ {sourceRate}Hz -> {targetSamples} samples @ {targetRate}Hz " +
        ...
        private void AnalyzeAndLogPcmLevelShort(short[] pcmSamples, string label, bool showFirstSamples = false)
        {
            ...
            AppLog.Log($"[RtpCallRecorder] PCM level ({label}): peak={peak}, rms={rms}, samples={pcmSamples.Length}{firstSamplesStr}");
        }
        ...
        public override async Task StopRecordingAsync(DateTime? callEndTime = null)
        {
            ...
            try
            {
                lock (_lockObject)
                {
                    if (_pcmInStream != null)
                    {
                        ...
                        _pcmInStream.Dispose();
                        _pcmInStream = null;
                        AppLog.Log($"[RtpCallRecorder] PCM inbound file closed: {_pcmInPath}");
                    }
                    ...
                    if (_pcmOutStream != null)
                    {
                        ...
                        _pcmOutStream = null;
                        AppLog.Log($"[RtpCallRecorder] PCM outbound file closed: {_pcmOutPath}");
                    }
                }
            }
            catch (Exception ex)
            {
                AppLog.Log($"[RtpCallRecorder] Error closing PCM files: {ex.Message}");
            }
            ...
            if (!string.IsNullOrEmpty(_pcmInPath) && !string.IsNullOrEmpty(_recordingFilePath))
            {
                ...
                if (hasInbound)
                {
                    ...
                    AppLog.Log($"[RtpCallRecorder] Inbound PCM file: {Path.GetFileName(_pcmInPath)}, size={inboundSize} bytes ({inboundSize / 1024} KB)");
                }
                else
                {
                    AppLog.Log($"[RtpCallRecorder] WARNING: Inbound PCM file not found: {_pcmInPath}");
                }
                
                if (hasOutbound && !string.IsNullOrEmpty(_pcmOutPath))
                {
                    outboundSize = new FileInfo(_pcmOutPath).Length;
                    AppLog.Log($"[RtpCallRecorder] Outbound PCM file: {Path.GetFileName(_pcmOutPath)}, size={outboundSize} bytes ({outboundSize / 1024} KB), chunks recorded={_outboundPcmChunksSeen}");
                }
                else
                {
                    AppLog.Log($"[RtpCallRecorder] WARNING: Outbound PCM file not found: {_pcmOutPath}, chunks recorded={_outboundPcmChunksSeen}");
                }
                
                // ИТОГОВАЯ СВОДКА для диагностики
                AppLog.Log($"[RtpCallRecorder] === RECORDING SUMMARY ===");
                AppLog.Log($"[RtpCallRecorder] Inbound: {inboundSize} bytes ({inboundSize / 1024} KB), RTP packets: {_inboundRtpPacketsSeen}");
                AppLog.Log($"[RtpCallRecorder] Outbound: {outboundSize} bytes ({outboundSize / 1024} KB), RTP packets: {_outboundRtpPacketsSeen}, PCM chunks: {_outboundPcmChunksSeen}, processed: {_outboundRtpPayloadProcessed}");
                AppLog.Log($"[RtpCallRecorder] =========================");
                ...
                {
                    AppLog.Log($"[RtpCallRecorder] WARNING: Both PCM files are empty or missing, skipping conversion");
                    try
                    {
                        if (File.Exists(_pcmInPath)) File.Delete(_pcmInPath);
                        if (!string.IsNullOrEmpty(_pcmOutPath) && File.Exists(_pcmOutPath)) File.Delete(_pcmOutPath);
                    }
                    catch { /* ignore */ }
                    _recordingFilePath = null;
                    return;
                ...
                {
                    AppLog.Log($"[RtpCallRecorder] Mixing inbound and outbound PCM files into WAV...");
                    ...
                    {
                        AppLog.Log($"[RtpCallRecorder] WARNING: mixing failed, falling back to inbound‑only WAV...");
                        converted = await ConvertPcmToWavWithFfmpegAsync(_pcmInPath, _recordingFilePath);
                    }
                }
                else if (hasInbound)
                {
                    AppLog.Log($"[RtpCallRecorder] Converting inbound PCM to WAV (outbound file missing or empty)...");
                    converted = await ConvertPcmToWavWithFfmpegAsync(_pcmInPath, _recordingFilePath);
                }
                else if (hasOutbound && !string.IsNullOrEmpty(_pcmOutPath))
                {
                    AppLog.Log($"[RtpCallRecorder] Converting outbound PCM to WAV (inbound file missing or empty)...");
                    converted = await ConvertPcmToWavWithFfmpegAsync(_pcmOutPath, _recordingFilePath);
                }
                
                if (converted)
                {
                    AppLog.Log($"[RtpCallRecorder] Final WAV ready: {_recordingFilePath}");
                    ...
                    try
                    {
                        ...
                        AppLog.Log($"[RtpCallRecorder] PCM files deleted");
                    }
                    catch (Exception ex)
                    {
                        AppLog.Log($"[RtpCallRecorder] Warning: Could not delete PCM files: {ex.Message}");
                    }
                }
                else
                {
                    AppLog.Log($"[RtpCallRecorder] WARNING: ffmpeg conversion failed, PCM files kept");
            ...
            {
                AppLog.Log($"[RtpCallRecorder] WARNING: Cannot convert - PCM paths or WAV path not set");
                _recordingFilePath = null;
            }
        ...
        private async Task<bool> MixPcmFilesWithFfmpegAsync(string inboundPcmPath, string outboundPcmPath, string wavPath)
        {
            ...
            {
                AppLog.Log($"[RtpCallRecorder] PCM files not found for mixing");
                return false;
            }
            ...
            {
                AppLog.Log($"[RtpCallRecorder] ffmpeg not found, skipping mixing. PCM files will be kept.");
                return false;
            }
        ...
        private async Task<bool> ConvertPcmToWavWithFfmpegAsync(string pcmPath, string wavPath)
        {
            ...
            {
                AppLog.Log($"[RtpCallRecorder] PCM file not found: {pcmPath}");
                return false;
            }
            ...
            if (string.IsNullOrEmpty(ffmpegPath))
            {
                AppLog.Log($"[RtpCallRecorder] ffmpeg not found, skipping conversion. PCM file will be kept: {pcmPath}");
                return false;
            }
        ...
        public static async Task<bool> TryRecoverWavFromOrphanPcmAsync(string wavPath)
        {
            ...
            try
            {
                ...
                AppLog.Log($"[RtpCallRecorder] Recovering orphan PCM → WAV: {Path.GetFileName(wavPath)} (in={hasIn}, out={hasOut})");
                ...
                {
                    AppLog.Log($"[RtpCallRecorder] Orphan PCM recovery OK: {wavPath}");
                    try
                    {
                ...
                AppLog.Log($"[RtpCallRecorder] Orphan PCM recovery failed for {Path.GetFileName(wavPath)}");
                return false;
            }
            catch (Exception ex)
            {
                AppLog.Log($"[RtpCallRecorder] Orphan PCM recovery error: {ex.Message}");
                return false;
            }
        }
        ...
        public static bool TryRecoverWavFromOrphanPcm(string wavPath, int timeoutMs = 45000)
        {
            try
            {
                var task = TryRecoverWavFromOrphanPcmAsync(wavPath);
            ...
            {
                AppLog.Log($"[RtpCallRecorder] Orphan PCM recovery sync wait failed: {ex.Message}");
                return false;
            }
        }
    }
