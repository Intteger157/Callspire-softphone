    ...
    public sealed class TapAudioSource : IAudioSource
    {
        ...
        public Task StartAudio()
        {
            lock (_lockObject)
            {
                ...
                MainWindow.Log($"[TapAudioSource] Subscribed to inner source ({_inner.GetType().Name}), starting audio...");
            }
        ...
        public Task CloseAudio()
        {
            lock (_lockObject)
            {
                if (_started)
                {
                    ...
                    _inner.OnAudioSourceEncodedSample -= Inner_OnAudioSourceEncodedSample;
                    _started = false;
                    MainWindow.Log("[TapAudioSource] Unsubscribed from inner source");
                }
            }
        ...
        private void Inner_OnAudioSourceRawSample(AudioSamplingRatesEnum rate, uint durationMs, short[] samples)
        {
            ...
            if (n == 1 || n % 50 == 0)
            {
                ...
                MainWindow.Log($"[TapAudioSource] RAW #{n}: rate={rate}, dur={durationMs}ms, samples={samples?.Length ?? 0}, peak={peak}");
            }
            ...
            {
                MainWindow.Log($"[TapAudioSource] ERROR in OnTapRawSample: {ex.Message}");
            }
            ...
            {
                MainWindow.Log($"[TapAudioSource] ERROR invoking OnAudioSourceRawSample: {ex.Message}");
            }
        }
