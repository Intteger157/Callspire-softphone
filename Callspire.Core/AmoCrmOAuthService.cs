    ...
    public class AmoCrmOAuthService
    {
        ...
        public async Task<(string? code, string? referer)> AuthorizeAsync(string subdomain, string clientId, string redirectUri, string? state = null)
        {
            try
            {
                ...
                AppLog.Log($"[AmoCrmOAuthService] Starting OAuth authorization...");
                AppLog.Log($"[AmoCrmOAuthService] Authorization URL: {authUrl}");
                AppLog.Log($"[AmoCrmOAuthService] Note: Authorization always uses www.amocrm.ru, subdomain will be returned in 'referer' parameter");
                ...
                {
                    AppLog.Log("[AmoCrmOAuthService] Failed to start local HTTP server");
                    return (null, null);
                }
                ...
                if (localRedirectUri != redirectUri)
                {
                    ...
                    AppLog.Log($"[AmoCrmOAuthService] Using local redirect URI: {localRedirectUri}");
                }
                ...
                try
                {
                    ...
                    AppLog.Log("[AmoCrmOAuthService] Browser opened, waiting for authorization...");
                }
                catch (Exception ex)
                {
                    AppLog.Log($"[AmoCrmOAuthService] Failed to open browser: {ex.Message}");
                    StopLocalServer();
                    return (null, null);
                }
                ...
                {
                    AppLog.Log("[AmoCrmOAuthService] Authorization timeout (5 minutes)");
                    StopLocalServer();
                    return (null, null);
                }
                ...
                if (result.code != null)
                {
                    AppLog.Log($"[AmoCrmOAuthService] Authorization code received successfully");
                    AppLog.Log($"[AmoCrmOAuthService] Referer: {result.referer}");
                }
                else
                {
                    AppLog.Log("[AmoCrmOAuthService] Authorization failed or was cancelled");
                }
            ...
            {
                AppLog.Log($"[AmoCrmOAuthService] Error during authorization: {ex.Message}");
                StopLocalServer();
                return (null, null);
            }
        }
        ...
        public async Task<(string? accessToken, string? refreshToken, int? expiresIn)> ExchangeCodeForTokensAsync(string subdomain, string clientId, string clientSecret, string authorizationCode, string redirectUri)
        {
            try
            {
                ...
                AppLog.Log($"[AmoCrmOAuthService] Exchanging authorization code for tokens...");
                AppLog.Log($"[AmoCrmOAuthService] Token URL: {tokenUrl}");
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        AppLog.Log($"[AmoCrmOAuthService] Token exchange failed: {response.StatusCode} - {responseContent}");
                    ...
                    if (accessToken != null)
                    {
                        AppLog.Log($"[AmoCrmOAuthService] Tokens received successfully (expires in {expiresIn} seconds)");
                    }
                    else
                    {
                        AppLog.Log("[AmoCrmOAuthService] Access token not found in response");
                    }
            ...
            {
                AppLog.Log($"[AmoCrmOAuthService] Error exchanging code for tokens: {ex.Message}");
                return (null, null, null);
            }
        }
        ...
        public async Task<(string? accessToken, string? refreshToken, int? expiresIn)> RefreshTokenAsync(string subdomain, string clientId, string clientSecret, string refreshToken, string redirectUri)
        {
            try
            {
                ...
                AppLog.Log($"[AmoCrmOAuthService] Refreshing access token...");
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                        AppLog.Log($"[AmoCrmOAuthService] Token refresh failed: {response.StatusCode} - {responseContent}");
                        return (null, null, null);
                    }
                    ...
                    if (accessToken != null)
                    {
                        AppLog.Log($"[AmoCrmOAuthService] Token refreshed successfully (expires in {expiresIn} seconds)");
                    }
                    else
                    {
                        AppLog.Log("[AmoCrmOAuthService] Access token not found in refresh response");
                    }
            ...
            {
                AppLog.Log($"[AmoCrmOAuthService] Error refreshing token: {ex.Message}");
                return (null, null, null);
            }
        }
        ...
        private async Task<string?> StartLocalServerAsync(string redirectUri)
        {
            try
            {
                ...
                {
                    AppLog.Log($"[AmoCrmOAuthService] Invalid redirect URI format: {redirectUri}");
                    return null;
                }
                ...
                lock (_lockObject)
                {
                    ...
                    try
                    {
                        _httpListener.Start();
                        _isListening = true;
                        AppLog.Log($"[AmoCrmOAuthService] Local HTTP server started on port {port}, path: {path}");
                    }
                    catch (HttpListenerException ex)
                    {
                        AppLog.Log($"[AmoCrmOAuthService] Failed to start HTTP server on port {port}: {ex.Message}");
                        AppLog.Log($"[AmoCrmOAuthService] Error code: {ex.ErrorCode}. Port may be in use or requires admin rights.");
                        ...
                        if (ex.ErrorCode == 32 || ex.ErrorCode == 183) // Port already in use
                        {
                            for (int tryPort = 8081; tryPort <= 8090; tryPort++)
                            {
                                try
                                {
                                    ...
                                    _isListening = true;
                                    port = tryPort;
                                    AppLog.Log($"[AmoCrmOAuthService] Started HTTP server on alternative port {tryPort}");
                                    break;
                                }
                                catch
                                {
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    catch (Exception ex)
                    {
                        if (_isListening)
                        {
                            AppLog.Log($"[AmoCrmOAuthService] Error in HTTP listener: {ex.Message}");
                        }
                    }
                });
            ...
            {
                AppLog.Log($"[AmoCrmOAuthService] Error starting local server: {ex.Message}");
                return null;
            }
        }
        ...
        private async Task HandleRequestAsync(HttpListenerContext context)
        {
            try
            {
                ...
                AppLog.Log($"[AmoCrmOAuthService] Received request: {request.Url}");
                AppLog.Log($"[AmoCrmOAuthService] Request path: {request.Url?.AbsolutePath}");
                ...
                AppLog.Log($"[AmoCrmOAuthService] Query params: code={(!string.IsNullOrEmpty(queryParams["code"]) ? "present" : "missing")}, referer={queryParams["referer"]}, error={queryParams["error"]}");
                ...
                {
                    AppLog.Log($"[AmoCrmOAuthService] Authorization error: {error}");
...
<body>
    ...
    <p>You can close this window.</p>
                ...
                if (!string.IsNullOrEmpty(code))
                {
                    ...
                    AppLog.Log($"[AmoCrmOAuthService] Authorization code received: {code.Substring(0, Math.Min(20, code.Length))}...");
...
<body>
    ...
    <p>This endpoint is used for OAuth authorization.</p>
            ...
            {
                AppLog.Log($"[AmoCrmOAuthService] Error handling request: {ex.Message}");
                try
                {
            ...
            lock (_lockObject)
            {
                if (_httpListener != null && _isListening)
                {
                    try
                    {
                        ...
                        _httpListener.Stop();
                        _httpListener.Close();
                        AppLog.Log("[AmoCrmOAuthService] Local HTTP server stopped");
                    }
                    catch (Exception ex)
                    {
                        AppLog.Log($"[AmoCrmOAuthService] Error stopping server: {ex.Message}");
                    }
                    finally
                    {
                        _httpListener = null;
