        ...
        public static IntPtr Create(int sampleRate, int channels, int frameMs)
        {
            ...
            if (!Avx2.IsSupported)
            {
                if (!_loggedCpuUnsupported)
                {
                    ...
                    AppLog.Log("[NativeAec] webrtc_apm.dll is built with AVX2; this CPU has no AVX2 — skipping native AEC (no illegal-instruction crash). SIP calls work with echo cancellation off.");
                }
                return IntPtr.Zero;
            }
