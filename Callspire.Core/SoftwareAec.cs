...
namespace Softphone
{
    ...
    public sealed class SoftwareAec
    {
        ...
        private void LogFilterState(string label)
        {
            ...
            AppLog.Log($"[SoftwareAec] [{label}] peak tap={peakK}/{_filterLen} (echo age ~{peakAge:F1}ms), " +
        ...
        private void EstimateDelay()
        {
            ...
            if (refEnergy < 1e-4f)
            {
                ...
                AppLog.Log($"[SoftwareAec] Low reference energy ({refEnergy:E2}), using default delay " +
            ...
            AppLog.Log($"[SoftwareAec] Delay estimated: {bestLag} samples ({bestLag * 1000f / SAMPLE_RATE:F1}ms), " +
