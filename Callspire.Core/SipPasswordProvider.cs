...
namespace Softphone
{
    ...
    public static class SipPasswordProvider
    {
        ...
        public static string? GetMainWebRtcPassword(AppSettings? settings)
        {
            ...
            {
                AppLog.Log($"[SipPasswordProvider] WebRtcPasswordEncrypted decrypt failed ({DescribeEnc(settings.WebRtcPasswordEncrypted)}): {ex.Message}");
            }
            ...
            if (!string.IsNullOrEmpty(webRtc))
            {
                ...
                {
                    AppLog.Log("[SipPasswordProvider] WARNING: WebRTC and SIP passwords differ in settings.json — WebRTC field is used. Re-save connection settings to sync.");
                }
                return webRtc;
            }
            ...
            {
                AppLog.Log("[SipPasswordProvider] WebRTC password unavailable; using SIP password fallback.");
            }
        ...
        public static void MigratePlaintextToEncryptedIfNeeded(string settingsFilePath, AppSettings settings)
        {
            ...
            {
                AppLog.Log($"[SipPasswordProvider] Error migrating SIP password to encrypted: {ex.Message}");
            }
        }
    }
}
