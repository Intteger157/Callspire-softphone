...
namespace Softphone
{
    ...
    public class FilteringAudioSink : IAudioSink
    {
        ...
        public void SetNegotiatedPayloadType(int payloadType)
        {
            ...
            AppLog.Log($"[FilteringAudioSink] {_label}Added negotiated PT {payloadType} to allowed set");
        }
        ...
        public void GotAudioRtp(IPEndPoint remoteEndPoint, uint ssrc, uint seqnum, uint timestamp, int payloadID, bool marker, byte[]? payload)
        {
            ...
            else
            {
                ...
                if (!_loggedFirstFiltered)
                {
                    ...
                    AppLog.Log($"[FilteringAudioSink] {_label}FILTERED first non-audio RTP packet: PT={payloadID} ({ptName}), len={payload?.Length ?? 0}. " +
