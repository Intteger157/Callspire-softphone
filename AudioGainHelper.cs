...
namespace Softphone
{
    ...
    public class AmplifiedAudioSource : IAudioSource
    {
        ...
        {
            MainWindow.Log($"[AmplifiedAudioSource] StartAudio called, subscribing to base source events, current subscribers={OnAudioSourceRawSample?.GetInvocationList().Length ?? 0}");
            ...
            MainWindow.Log($"[AmplifiedAudioSource] Subscribed to base source events, calling base.StartAudio()");
            ...
            MainWindow.Log($"[AmplifiedAudioSource] StartAudio completed, subscribers after start={subscribersAfterStart}");
            ...
            result.ContinueWith(_ =>
            {
                ...
                MainWindow.Log($"[AmplifiedAudioSource] After StartAudio completion, current subscribers={currentSubscribers}");
                
                // Если подписчиков нет, это странно, но логируем
                if (currentSubscribers == 0)
                {
                    MainWindow.Log($"[AmplifiedAudioSource] WARNING: No subscribers after StartAudio completion!");
                }
        ...
        private void OnBaseSourceRawSample(AudioSamplingRatesEnum samplingRate, uint durationMilliseconds, short[] sample)
        {
            ...
            if (isFirstCall || _onBaseSourceRawSampleCallCount <= 10)
            {
                ...
                MainWindow.Log($"[AmplifiedAudioSource][MIC] OnBaseSourceRawSample called #{_onBaseSourceRawSampleCallCount}: " +
                ...
                if (sample != null && sample.Length > 0)
                {
                    ...
                    MainWindow.Log($"[AmplifiedAudioSource][MIC] Sample peak={peak}, first 3 samples: [{string.Join(", ", sample.Take(3))}]");
                }
            }
            else if (_onBaseSourceRawSampleCallCount % 100 == 0)
            {
                var subscribers = OnAudioSourceRawSample?.GetInvocationList();
                int subscriberCount = subscribers?.Length ?? 0;
                MainWindow.Log($"[AmplifiedAudioSource] OnBaseSourceRawSample called #{_onBaseSourceRawSampleCallCount} times, subscribers={subscriberCount}");
            }
            ...
            if (sample == null || sample.Length == 0)
            {
                ...
                {
                    MainWindow.Log($"[AmplifiedAudioSource] WARNING: OnBaseSourceRawSample called with null or empty sample");
                }
                return;
            }
            ...
            if (OnAudioSourceRawSample != null)
            {
                try
                {
                    ...
                    if (isFirstCall)
                    {
                        MainWindow.Log($"[AmplifiedAudioSource][MIC] OnAudioSourceRawSample.Invoke called successfully, subscribers notified");
                    }
                }
                catch (Exception ex)
                {
                    MainWindow.Log($"[AmplifiedAudioSource] ERROR invoking OnAudioSourceRawSample: {ex.Message}, stack: {ex.StackTrace}");
                }
            }
            else
            {
                if (_onBaseSourceRawSampleCallCount <= 5)
                {
                    MainWindow.Log($"[AmplifiedAudioSource] WARNING: OnBaseSourceRawSample called but OnAudioSourceRawSample has no subscribers!");
                }
            }
        }
