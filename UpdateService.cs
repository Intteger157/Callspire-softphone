    ...
    public class UpdateService
    {
        ...
        private static void SaveLastCheckTime(DateTime checkTime)
        {
            try
            {
                ...
                MainWindow.Log($"[UpdateService] Saved last check time: {checkTime}");
            }
            catch (Exception ex)
            {
                MainWindow.Log($"[UpdateService] Error saving last check time: {ex.Message}");
            }
        }
        ...
        private static DateTime? GetLastCheckTime()
        {
            try
            {
                ...
                if (data != null)
                {
                    ...
                    if (lastCheckTime != null)
                    {
                        ...
                        MainWindow.Log($"[UpdateService] Last check time: {lastCheck}");
                        return lastCheck;
                    }
                }
            }
            ...
            {
                MainWindow.Log($"[UpdateService] Error reading last check time: {ex.Message}");
            }
        ...
        public static bool ShouldCheckForUpdates()
        {
            ...
            if (lastCheck == null)
            {
                MainWindow.Log("[UpdateService] No previous check found, will check for updates");
            ...
            if (shouldCheck)
            {
                MainWindow.Log($"[UpdateService] Last check was {timeSinceLastCheck.TotalHours:F1} hours ago, will check for updates");
            }
            else
            {
                MainWindow.Log($"[UpdateService] Last check was {timeSinceLastCheck.TotalHours:F1} hours ago, skipping check (check interval: {UpdateCheckIntervalHours} hours)");
            }
        ...
        public static string GetCurrentVersion()
        {
            ...
            {
                MainWindow.Log($"[UpdateService] Error getting current version: {ex.Message}");
            }
        ...
        public static async Task<UpdateInfo?> CheckForUpdateAsync(bool forceCheck = false)
        {
            ...
            try
            {
                ...
                MainWindow.Log($"[UpdateService] Checking for updates: {url}");
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        MainWindow.Log($"[UpdateService] JSON parsing error: {jsonEx.Message}");
                        MainWindow.Log($"[UpdateService] JSON content (first 500 chars): {json.Substring(0, Math.Min(500, json.Length))}");
                        MainWindow.Log($"[UpdateService] Please check your update.json file on the server for syntax errors.");
                    ...
                    {
                        MainWindow.Log($"[UpdateService] Latest version found: {updateInfo.Version}");
                        ...
                        if (comparison < 0)
                        {
                            MainWindow.Log($"[UpdateService] New version available: {currentVersion} -> {updateInfo.Version}");
                            return updateInfo;
                        }
                        else
                        {
                            MainWindow.Log($"[UpdateService] Application is up to date. Current: {currentVersion}, Latest: {updateInfo.Version}");
                            return null;
                        }
                    }
                    else
                    {
                        MainWindow.Log("[UpdateService] Update info received but version field is empty");
                    }
                }
                else
                {
                    MainWindow.Log($"[UpdateService] Server returned status: {response.StatusCode}");
                    // Не сохраняем время проверки при ошибке, чтобы можно было повторить
                }
            }
            catch (TaskCanceledException)
            {
                MainWindow.Log("[UpdateService] Update check timed out");
                // Не сохраняем время проверки при таймауте
            }
            catch (HttpRequestException ex)
            {
                MainWindow.Log($"[UpdateService] Network error checking for updates: {ex.Message}");
                // Не сохраняем время проверки при сетевой ошибке
            }
            catch (Exception ex)
            {
                MainWindow.Log($"[UpdateService] Error checking for updates: {ex.Message}");
        ...
        public static string CalculateFileSha256(string filePath)
        {
            ...
            {
                MainWindow.Log($"[UpdateService] Error calculating SHA256: {ex.Message}");
                return string.Empty;
            }
        }
        ...
        public static bool VerifyFileHash(string filePath, string expectedHash)
        {
            ...
            {
                MainWindow.Log("[UpdateService] No SHA256 hash provided in update info, skipping verification");
            ...
            if (!isValid)
            {
                MainWindow.Log($"[UpdateService] SHA256 hash mismatch! Expected: {expectedHash}, Actual: {actualHash}");
            }
            else
            {
                MainWindow.Log("[UpdateService] SHA256 hash verification passed");
            }
