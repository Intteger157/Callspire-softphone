...
def _job_to_api(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if "pbx_was_answered" in payload:
        result["pbx_was_answered"] = bool(payload["pbx_was_answered"])
    pbx_duration = payload.get("pbx_duration_seconds")
    if pbx_duration is not None:
        try:
            result["pbx_duration_seconds"] = int(pbx_duration)
    ...
    async def kommo_process_call(body: ProcessCallBody, user: dict = Depends(require_jwt)):
        ...
        if job.get("id"):
            dropped = kommo_jobs_db.fail_superseded_queued_jobs(
                ext, body.phone, except_job_id=job["id"]
            )
            if dropped:
                print(
