...
Workflow `.github/workflows/release.yml` builds a **self-contained** Windows installer **`Callspire-<version>-win-x64-setup.exe`** (Inno Setup: .NET runtime, ffmpeg, WebRtcClient, optional `webrtc_apm.dll`). Tags with a hyphen (e.g. `v1.0.0-beta.1`) are marked as pre-releases.

The app is not a single loose `Callspire.exe` — it needs bundled DLLs and data folders; the setup exe installs everything. WebView2 Evergreen runtime must still be present on the PC (standard on Windows 10/11).
